<?php
/**
 * Blogs Page Banner Management (Multi-Banner)
 */
$pageTitle = "Blogs Page Banner Settings";
...
if ($action === 'delete' && $editId) {
    ...
    if ($stmt->execute()) {
        header("Location: banner-blogs.php?success=" . urlencode("Banner deleted successfully."));
        exit;
    } else {
    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_FILES['banner_image']) && $_FILES['banner_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($extension, $allowedExtensions)) {
                ...
                $newFilename = 'blogs-banner-' . uniqid() . '.' . $extension;
        ...
        if ($uploadOk) {
            if ($bannerId) {
                ...
                if ($stmt->execute()) {
                    header("Location: banner-blogs.php?success=" . urlencode('Banner updated successfully.'));
                    exit;
                } else {
                ...
                if ($stmt->execute()) {
                    header("Location: banner-blogs.php?success=" . urlencode('New banner added successfully.'));
                    exit;
                } else {
                    $error = 'Database error: ' . $conn->error;
                }
...
<div class="page-header">
    <div class="page-title-section">
        <h1 class="page-title">Blogs Banners</h1>
        <p class="page-subtitle">Manage hero section sliders</p>
    </div>
    <?php if ($action === 'list'): ?>
        <a href="banner-blogs.php?action=add" class="btn btn-primary">
    ...
    <?php else: ?>
        <a href="banner-blogs.php" class="btn btn-secondary">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                style="width: 18px; height: 18px; margin-right: 5px;">
                        ...
                        <?php foreach ($bannersList as $b): ?>
                            <tr style="border-bottom: 1px solid #e2e8f0;">
                                ...
                                <td style="padding: 1rem; text-align: right;">
                                    <a href="banner-blogs.php?action=edit&id=<?php echo $b['id']; ?>" class="btn-sm"
                                        style="color: #2563eb; margin-right: 10px; text-decoration: none;">Edit</a>
                                    <a href="banner-blogs.php?action=delete&id=<?php echo $b['id']; ?>" class="btn-sm"
