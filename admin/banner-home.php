<?php
/**
 * Home Banner Management (Multi-Banner)
 */
$pageTitle = "Home Banner Settings";
...
if ($action === 'delete' && $editId) {
    // Validate CSRF token via GET param (simplified for this context, ideally POST)
    // For safety, let's require a confirm step or just check referer/token if possible
    // Here we'll rely on the logged-in admin status and assume safe usage

    $stmt = $conn->prepare("DELETE FROM banner_settings WHERE id = ? AND page_name = ?");
    $stmt->bind_param("is", $editId, $pageName);
    if ($stmt->execute()) {
        header("Location: banner-home.php?success=" . urlencode("Banner deleted successfully."));
        exit;
    } else {
    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_FILES['banner_image']) && $_FILES['banner_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($extension, $allowedExtensions)) {
                ...
                $newFilename = 'home-banner-' . uniqid() . '.' . $extension;
        ...
        if ($uploadOk) {
            if ($bannerId) {
                // Update
                if (empty($imagePath)) {
                    // Check existing path if not uploading new
                ...
                if ($stmt->execute()) {
                    ...
                    // Redirect to list to show success
                    header("Location: banner-home.php?success=" . urlencode($success));
                    exit;
                } else {
...
// Fetch List
$bannersList = [];
$listQ = $conn->query("SELECT * FROM banner_settings WHERE page_name = 'home' ORDER BY sort_order ASC, created_at DESC");
if ($listQ) {
    while ($r = $listQ->fetch_assoc()) {
        $bannersList[] = $r;
    }
}
...
<div class="page-header">
    ...
    <?php if ($action === 'list'): ?>
        <a href="banner-home.php?action=add" class="btn btn-primary">
    ...
    <?php else: ?>
        <a href="banner-home.php" class="btn btn-secondary">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                style="width: 18px; height: 18px; margin-right: 5px;">
                    ...
                    <?php else: ?>
                        <?php foreach ($bannersList as $b): ?>
                            <tr style="border-bottom: 1px solid #e2e8f0;">
                                ...
                                <td style="padding: 1rem;">
                                    <div style="font-weight: 500; font-size: 0.95rem;"><?php echo strip_tags($b['heading_html']); ?>
                                    </div>
                                    <div style="font-size: 0.85rem; color: #777;"><?php echo $b['eyebrow_text']; ?></div>
                                </td>
                                <td style="padding: 1rem;"><?php echo $b['sort_order']; ?></td>
                                <td style="padding: 1rem;">
                                    <?php if ($b['is_active']): ?>
                                        <span
                                ...
                                <td style="padding: 1rem; text-align: right;">
                                    <a href="banner-home.php?action=edit&id=<?php echo $b['id']; ?>" class="btn-sm"
                                        style="color: #2563eb; margin-right: 10px; text-decoration: none;">Edit</a>
                                    <a href="banner-home.php?action=delete&id=<?php echo $b['id']; ?>" class="btn-sm"
...
<?php else: ?>
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            <h3><?php echo $editData['id'] ? 'Edit Banner' : 'Create New Banner'; ?></h3>
        </div>
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="eyebrow_text" id="eyebrow_text" class="form-control"
                        ...
                        placeholder="e.g., Factory Design Experts">
                </div>
