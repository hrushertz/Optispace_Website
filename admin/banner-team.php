<?php
/**
 * Team Page Banner Management (Multi-Banner)
 */
$pageTitle = "Team Page Banner Settings";
...
if ($action === 'delete' && $editId) {
    ...
    if ($stmt->execute()) {
        header("Location: banner-team.php?success=" . urlencode("Banner deleted successfully."));
        exit;
    } else {
    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_FILES['banner_image']) && $_FILES['banner_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($extension, $allowedExtensions)) {
                ...
                $newFilename = 'team-banner-' . uniqid() . '.' . $extension;
        ...
        if ($uploadOk) {
            if ($bannerId) {
                ...
                if ($stmt->execute()) {
                    header("Location: banner-team.php?success=" . urlencode('Banner updated successfully.'));
                    exit;
                } else {
                ...
                if ($stmt->execute()) {
                    header("Location: banner-team.php?success=" . urlencode('New banner added successfully.'));
                    exit;
                } else {
                    $error = 'Database error: ' . $conn->error;
                }
...
<div class="page-header">
    <div class="page-title-section">
        <h1 class="page-title">Team Banners</h1>
        <p class="page-subtitle">Manage hero section sliders</p>
    </div>
    <?php if ($action === 'list'): ?>
        <a href="banner-team.php?action=add" class="btn btn-primary">
    ...
    <?php else: ?>
        <a href="banner-team.php" class="btn btn-secondary">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                style="width: 18px; height: 18px; margin-right: 5px;">
                        ...
                        <?php foreach ($bannersList as $b): ?>
                            <tr style="border-bottom: 1px solid #e2e8f0;">
                                ...
                                <td style="padding: 1rem; text-align: right;">
                                    <a href="banner-team.php?action=edit&id=<?php echo $b['id']; ?>" class="btn-sm"
                                        style="color: #2563eb; margin-right: 10px; text-decoration: none;">Edit</a>
                                    <a href="banner-team.php?action=delete&id=<?php echo $b['id']; ?>" class="btn-sm"
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="eyebrow_text" id="eyebrow_text" class="form-control"
                        value="<?php echo htmlspecialchars($editData['eyebrow_text']); ?>" placeholder="e.g., Our Network">
                </div>
