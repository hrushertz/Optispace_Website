<?php
/**
 * Home Outcomes Image Management (Single Image)
 */
$pageTitle = "Home Outcomes Image";
...
$success = '';
$error = '';
$pageName = 'home_outcomes'; // Unique identifier for this single record
...
if ($res->num_rows > 0) {
    ...
    $existingData = [
        ...
        'heading_html' => 'what manufacturers achieve with LFB',
        'subheading' => 'Beyond the numbers, the operational impact is felt on the shop floor every day. Clients report significant reduction in internal material travel and handling effort, improved manufacturing throughput and space utilization, and better structural readiness for future automation.',
        'is_active' => 1
    ];
}
    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_FILES['process_image']) && $_FILES['process_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($extension, $allowedExtensions)) {
                ...
                $newFilename = 'outcomes-bg-' . uniqid() . '.' . $extension;
...
<div class="page-header">
    <div class="page-title-section">
        <h1 class="page-title">Home Outcomes Image</h1>
        <p class="page-subtitle">Manage the "What manufacturers achieve with LFB" section background</p>
    </div>
</div>
    ...
    <div class="admin-card-body">
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="form-group row">
                <div class="col-8">
                    <label for="process_image">Section Background Image (1920x850px)</label>
                    <input type="file" name="process_image" id="process_image" class="form-control" accept="image/*">
                    <small class="form-text text-muted">Upload a large image. Leave empty to keep current image.</small>
                    ...
                    <?php if (!empty($existingData['image_path'])): ?>
                        <div
                            ...
                            <img src="<?php echo htmlspecialchars('../' . $existingData['image_path']); ?>"
                                alt="Current Image" style="max-width: 300px; height: auto;">
                        </div>
                    <?php else: ?>
