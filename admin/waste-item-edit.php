...
$pageTitle = 'Edit Waste Item';

// Get database connection
$conn = getDBConnection();

// Get waste item ID
$wasteId = isset($_GET['id']) ? intval($_GET['id']) : 0;
...
// Fetch waste item
$stmt = $conn->prepare("SELECT * FROM waste_items WHERE id = ?");
$stmt->bind_param("i", $wasteId);
$stmt->execute();
$result = $stmt->get_result();
$wasteItem = $result->fetch_assoc();
$stmt->close();

if (!$wasteItem) {
    $_SESSION['error_message'] = 'Waste item not found';
    header('Location: waste-items.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title)) {
        ...
        $stmt = $conn->prepare("UPDATE waste_items SET title = ?, icon_svg = ?, problem_text = ?, solution_text = ?, impact_text = ?, is_active = ? WHERE id = ?");
        $stmt->bind_param("sssssii", $title, $iconSvg, $problemText, $solutionText, $impactText, $isActive, $wasteId);
        
        if ($stmt->execute()) {
            $_SESSION['success_message'] = 'Waste item updated successfully';
            header('Location: waste-items.php');
            exit;
        } else {
            $formError = 'Error updating waste item: ' . $conn->error;
        }
        $stmt->close();
    }
} else {
...
<div class="admin-page">
    <div class="page-header">
        <div>
            ...
            <p class="page-subtitle">Update waste item details</p>
        </div>
    ...
    <div class="content-card">
        <form method="post" class="admin-form">
            ...
            <div style="margin-top: 1.5rem; padding-top: 1.5rem; border-top: 1px solid #E2E8F0; font-size: 0.85rem; color: #64748B;">
                <div><strong>Created:</strong> <?php echo date('M d, Y \a\t g:i A', strtotime($wasteItem['created_at'])); ?></div>
                <?php if ($wasteItem['updated_at'] !== $wasteItem['created_at']): ?>
                    <div><strong>Last Updated:</strong> <?php echo date('M d, Y \a\t g:i A', strtotime($wasteItem['updated_at'])); ?></div>
                <?php endif; ?>
            </div>
...
$(document).ready(function() {
    ...
    // Pre-select icon if exists
    const currentIconSvg = $('#icon_svg').val();
    if (currentIconSvg) {
        $('.icon-option').each(function() {
            const iconName = $(this).data('icon');
            if (iconSVGs[iconName] === currentIconSvg) {
                $(this).addClass('selected');
                $('#selectedIconDisplay').html(iconSVGs[iconName]);
